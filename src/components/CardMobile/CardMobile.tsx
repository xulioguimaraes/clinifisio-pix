    ...
    return (
        <section className={styles.container}>
            ...
            {transaction?.map(item => {
                return <div key={item.id} onClick={() => handleTransaction(item)}>
                    <h3>{item.title}</h3>
                    <h1 className={item.type ? styles.deposit : styles.withraw}
                    >{item.type ? "" : `- `}
                        {new Intl.NumberFormat("pt-BR", { style: 'currency', currency: 'BRL' }).format(item.price / 100)}</h1>
                    <div>
                        <label>{item.description}</label>
                        <label>{new Intl.DateTimeFormat("pt-BR").format(Date.parse(item.createdAt))}</label>
                    </div>
                </div>
            })}
